...
TRBtree::Deserialize(std::ifstream &input, TNode *&root) {
    ...
    bool color = RED;
    if (input.peek() == EOF ||
        (input >> buffer && !strcmp(buffer, "-1"))) { //peek смотрит на следующий символ потока, но не берет его
        return;
    }
...
void TRBtree::PrintTree() {
    PrintTree(root, 0);
}

void TRBtree::PrintTree(TNode *node, uint64_t depth) {
    if(node == nil) {
        return;
    }
